            ...
            <li key={tab.id}>
              <button
                className={`w-full p-4 text-left ${
                  activeTab === tab.id ?  'bg-black px-5 py-5 text-white border-0 rounded-2xl flex flex-col':'bg-white text-slate-800 border-[1px] border-gray-200 rounded-2xl px-5 py-5 flex flex-col'  
                }`}
                onClick={() => handleClick(tab.id)}
              >
                {tab.label} <FaAngleRight className="mr-2 ml-2 h-4 w-4" />
              </button>
            </li>
          ))}
        </ul>
