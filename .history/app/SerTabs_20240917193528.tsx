        ...
        <div className="w-1/4">
          <ul className="list-none p-0 m-0 flex flex-col gap-5">
            {tabs.map((tab) => (
              <li key={tab.id}>
                <button
                  className={`w-full p-4 text-left flex items-center gap-2 ${
                    activeTab === tab.id
                      ? 'bg-black text-white border-0 rounded-2xl'
                      : 'bg-white text-slate-800 border-[1px] border-gray-200 rounded-2xl'
                  }`}
